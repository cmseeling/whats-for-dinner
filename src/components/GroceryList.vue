    ...
    <ul>
      ...
      <v-container v-if="isAddingItem" tag="li" class="pa-0 add-ingredient-inputs" fill-height>
        <v-layout align-center>
          ...
          <v-flex shrink>
            <v-btn class="add-ingredient-cancel" color="error" @click="toggleAddItem">Cancel</v-btn>
          </v-flex>
        </v-layout>
      </v-container>
      <li v-if="!isAddingItem">
        <v-btn class="add-ingredient-button" color="success" @click.prevent="toggleAddItem"><v-icon small>fa fa-plus</v-icon>&nbsp;Add Item</v-btn>
      </li>
    </ul>
  </v-card-text>
</template>
